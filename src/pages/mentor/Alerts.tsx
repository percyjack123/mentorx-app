...
import { mentorApi, type MentorAlerts, type Student, type SosAlert } from "@/lib/api";
...
export default function Alerts() {
  const [alerts, setAlerts] = useState<MentorAlerts | null>(null);
  ...
  const alertSections = [
    {
      ...
      items: (alerts?.highRisk ?? []).map(
        (s: Student): AlertItem => ({
          label: s.name,
          detail: `Risk Score: ${s.risk_score}`,
          color: "border-danger/30 bg-danger/5",
        })
      ),
    },
    {
      title: "Tampered Documents",
      icon: <FileWarning className="h-5 w-5 text-warning" />,
      items: (alerts?.tamperedDocuments ?? []).map(
        (s): AlertItem => ({
          label: s.name,
    ...
    },
    {
      ...
      items: (alerts?.missedCheckins ?? []).slice(0, 8).map(
        (s: Student): AlertItem => ({
          label: s.name,
          detail: `Last: ${s.last_check_in ?? "Never"}`,
            ...
            <div className="space-y-2">
              {section.items.length > 0 ? (
                section.items.map((item, i) => (
                  <div
                    key={i}
                    className={`p-3 rounded-lg border cursor-pointer hover:shadow-sm transition-all ${item.color}`}
                  >
