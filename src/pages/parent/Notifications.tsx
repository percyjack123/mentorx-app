  ...
  const alertSections = [
    {
      ...
      items: alerts?.highRisk?.map((s: any) => ({
        label: s.name,
    ...
    },
    {
      ...
      items: alerts?.lowAttendance?.map((s: any) => ({
        label: s.name,
    ...
    },
    {
      ...
      items: alerts?.missedCheckins?.map((s: any) => ({
        label: s.name,
          ...
          <div key={section.title} className="rounded-xl border bg-card p-6">
            ...
            <div className="space-y-2">
              {section.items.length > 0 ? (
                ...
                <p className="text-sm text-muted-foreground">{section.emptyMessage || "None"}</p>
              )}
            </div>
          </div>
        ))}
