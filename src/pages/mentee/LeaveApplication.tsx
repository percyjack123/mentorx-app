...
import { leaveRecords, students } from "@/data/mockData";
...
export default function LeaveApplication() {
  ...
  const leaves = leaveRecords.filter(l => l.studentId === student.id);
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="rounded-xl border bg-card p-6 space-y-4">
        ...
        <Button type="submit" className="gradient-primary text-primary-foreground">Submit Application</Button>
      </form>
            ...
            <tbody>
              {leaves.map(l => (
                <tr key={l.id} className="border-b last:border-0">
                  <td className="py-3 px-2">{l.fromDate} to {l.toDate}</td>
