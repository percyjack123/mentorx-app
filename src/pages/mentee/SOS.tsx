...
import { AlertOctagon, Phone, Heart, Shield } from "lucide-react";
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto text-center">
      ...
      <div className="rounded-2xl border bg-card p-12 flex flex-col items-center gap-6">
        <button
          onClick={handleSOS}
          className="w-40 h-40 rounded-full bg-danger hover:bg-danger/90 text-danger-foreground flex flex-col items-center justify-center transition-all duration-200 hover:scale-105 shadow-lg hover:shadow-xl animate-pulse-soft"
        >
          <AlertOctagon className="h-12 w-12 mb-2" />
          <span className="text-lg font-bold font-display">🚨 SOS</span>
        </button>
        <p className="text-sm text-muted-foreground max-w-xs">Press the button above to send a high-priority alert to your mentor and emergency contacts.</p>
      </div>

      <div className="rounded-xl border bg-card p-6 text-left">
        <h3 className="font-semibold font-display mb-4 flex items-center gap-2"><Shield className="h-5 w-5 text-primary" /> Emergency Information</h3>
        <div className="grid gap-3">
          ...
          <div className="flex items-center gap-3 p-3 rounded-lg bg-accent/50">
            ...
            <div>
              ...
              <p className="font-medium text-sm">{student.chronicConditions}</p>
            </div>
          </div>
        </div>
      </div>
