...
export default function Feedback() {
  ...
  const avgRating = entries.length
    ? +(entries.reduce((s, f) => s + f.rating, 0) / entries.length).toFixed(1)
    : 0;
  const mentorsReviewed = new Set(entries.map((f) => f.mentor_id)).size;

  const byMentor = entries.reduce<Record<number, MentorGroup>>((acc, f) => {
    if (!acc[f.mentor_id]) {
      acc[f.mentor_id] = {
        name: f.mentor_name ?? "Unknown",
  ...
  const mentorRatings: MentorGroup[] = Object.values(byMentor).map((m) => ({
    ...m,
    avgRating: +(m.entries.reduce((s, e) => s + e.rating, 0) / m.entries.length).toFixed(1),
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <StatsCard
          ...
          value={mentorsReviewed}
          icon={<Star className="h-5 w-5 text-secondary" />}
        />
      </div>
      <div className="rounded-xl border bg-card overflow-hidden">
        <div className="p-4 border-b">
          <h3 className="font-semibold font-display">Mentor Ratings</h3>
        </div>
          ...
          {entries.map((f) => (
            <div key={f.id} className="p-4 rounded-lg bg-muted/50">
              ...
              <p className="text-xs text-muted-foreground mt-2">
                {new Date(f.created_at).toLocaleDateString()} • Anonymous
              </p>
            </div>
          ))}
          {entries.length === 0 && (
